L.Control.Elevation = L.Control.extend({
    ...
    _decimalAdjust: function(value, exp) {
    ...
    _addGeoJSONData: function(coords) {
        if (coords) {
            ...
            var min_ele = this.MinElevation || 100000;
            var up = 0,
                down = 0;
            for (var i = 0; i < coords.length; i++) {
                ...
                var change = coords[i][2] - coords[i ? i - 1 : 0][2];
                if (change>0) {up+=change;} else{down+=change;}
                dist = dist + Math.round(newdist / 1000 * 100000) / 100000;
                ele = ele < coords[i][2]*3.3 ? coords[i][2]*3.3 : ele;
                min_ele = min_ele > coords[i][2]*3.3 ? coords[i][2]*3.3 : min_ele;
                data.push({
                    dist: dist*0.62,
                    altitude: coords[i][2]*3.3,
                    x: coords[i][0],
                    y: coords[i][1],
                    latlng: s
                });
            ...
            this.MaxElevation = Math.round(this._maxElevation);
            this.MinElevation = Math.round(min_ele);
            this.TotalDist = this._decimalAdjust(this._dist*0.62,-1);
            this.up = Math.round(up*3.3);
            this.down = Math.abs(Math.round(down*3.3));
        }
    },
    ...
    _addGPXdata: function(coords) {
        if (coords) {
            ...
            this._maxElevation = ele;
            this.MaxElevation = Math.round(this._maxElevation);
            this.TotalDist = Math.round(this._dist);
        }
    },
